...
function RSSImport(
		...
		$before_date = ' ', $date = 0, $after_date = '', $date_format = '',
		$before_creator = ' ', $creator = 0, $after_creator = '',
...
function RSSImport_insert_button_old() {
	...
	<script type="text/javascript">
		...
		if ( typeof edButtons != \'undefined\' ) {
			...
			edButtons[length] = new edButton(\'RSSImport\', \'$context\', \'[RSSImport display="5" feedurl="http://feedurl.com/" before_desc="<br />" displaydescriptions="TRUE" after_desc=" " html="FALSE" truncatedescchar="200" truncatedescstring=" ... " truncatetitlechar=" " truncatetitlestring=" ... " before_date=" " date="FALSE" after_date="" date_format="" before_creator=" " creator="FALSE" after_creator="</small>" start_items="<ul>" end_items="</ul>" start_item="<li>" end_item="</li>" target="" rel="" desc4title="" charsetscan="FALSE" debug="FALSE" before_noitems="<p>" noitems="No items, feed is empty." after_noitems="</p>" before_error="<p>" error="Error: Feed has a error or is not valid" after_error="</p>" paging="FALSE" prev_paging_link="&laquo; Previous" next_paging_link="Next &raquo;" prev_paging_title="more items" next_paging_title="more items" use_simplepie="FALSE"]\', \'\', \'\');
...
if ( class_exists('WP_Widget') ) {
	...
	class RSSImport_Widget extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			$before_date         = empty($instance['before_date']) ? ' ' : $instance['before_date'];
			$date                = empty($instance['date']) ? '0' : $instance['date'];
			$after_date          = empty($instance['after_date']) ? '' : $instance['after_date'];
			$date_format         = empty($instance['date_format']) ? '' : $instance['date_format'];
			$before_creator      = empty($instance['before_creator']) ? ' ' : $instance['before_creator'];
			$creator             = empty($instance['creator']) ? '0' : $instance['creator'];
			$after_creator       = empty($instance['after_creator']) ? '' : $instance['after_creator'];
